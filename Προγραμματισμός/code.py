...
hl = HuskyLensLibrary('UART', TX=board.GP0, RX=board.GP1)  # Επικοινωνία με τη Huskylens με το πρωτόκολλο UART
hl.algorithm("ALGORITHM_OBJECT_TRACKING")  # Η Huskylens λειτουργεί σε mode "Object Tracking"
...
def read_HuskyLens():
    ...
    results = hl.learnedBlocks() # Ελέγχει αν εντοπίστηκε αντικέιμενο
    
    if results:                  # Αν εντοπίστηκε το αντικείμενο
        fish_detected = True     # Βοηθητική μεταβλητή
        r = results[0]           # Επιστρέφει πληροφορίες για τη θέση και το μέγεθος του αντικειμένου
        return r.y               # Η τετμημένη του αντικειμένου με y=0 στο επάνω μέρος της οθόνης 
    else:
        fish_detected = False
        return 0
...
def lcd_message(message):
    if message == 1:
        lcd.clear()                    # Διαγραφή ενδείξεων
        lcd.print("Button 1 pressed")  # Εμφάνιση νέου μηνύματος
        lcd.set_cursor_pos(1,0)        # Ορισμός της θέσης (x,y) του κέρσορα
        ...
        lcd.set_cursor_pos(1,0)
        lcd.print("Depth: ")
        lcd.print(str(fish_depth))  # Εμφάνιση τιμής βάθους αφού μετατραπεί σε σειρά από χαρακτήρες
        lcd.print("m")
    elif message == 5:
        lcd.clear()
        ...
        lcd.set_cursor_pos(1,0)
        fish = random.choice(fishes)  # Τυχαία επιλογή από τη λίστα με τα ψάρια
        lcd.print(fish)
    elif message == 7:
        lcd.clear()
...
def pull_hook():
    global hook_depth
    # Το servo περιστρέφεται αριστερόστροφα εκτός αν το αγκίστρι έφτασε στην επιφάνεια
...
def release_hook():
    global hook_depth
    # Το servo περιστρέφεται δεξιόστροφα εκτός αν το αγκίστρι έφτασε στον βυθό
...
def surface_pull_hook():
    ...
    # Το servo περιστρέφεται αριστερόστροφα μέχρι το αγκίστρι να φτάσει στην επιφάνεια
...
# Συνάρτηση που καθορίζει τυχαία αν πιάστηκε το ψάρι
def wait_for_catch():
    stop_hook()
    lcd_message(5)
    sleep(5)
...
while True:
    ...
    # Αν εντοπιστεί ψάρι που είναι πιο κάτω από το αγκίστρι
    if difference > 0 and fish_detected == True:
        lcd_message(4)
        # Το αγκίστρι κατεβαίνει μέχρι το ψάρι
        while difference != 0:
            release_hook()
            difference = fish_depth - hook_depth  # Υπολογισμός νέας απόστασης
        wait_for_catch()
    # Αν εντοπιστεί ψάρι που είναι πιο πάνω από το αγκίστρι
    elif difference < 0 and fish_detected == True:
        lcd_message(4)
        # Το αγκίστρι ανεβαίνει μέχρι το ψάρι
        while difference != 0:
            pull_hook()
            difference = fish_depth - hook_depth  # Υπολογισμός νέας απόστασης
        wait_for_catch()
